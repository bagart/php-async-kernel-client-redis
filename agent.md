...
The sync path exists because queue/cache/locker infrastructure classes need synchronous Redis access. They implement
contracts from `bagart/async-kernel` and `bagart/ask-client`.
...
`ASKRedisGetOperation`, `ASKRedisSetOperation`, `ASKRedisDelOperation`, `ASKRedisExpireOperation`,
`ASKRedisIncrOperation`, `ASKRedisPublishOperation`, `ASKRedisSubscribeOperation`.
...
`RedisClientContract` — comprehensive interface (~50 methods: strings, lists, hashes, sets, sorted sets, streams,
scripting, scan, pipeline).
...
Classes implementing contracts from `bagart/async-kernel` and `bagart/ask-client`. All use the sync path
(`RedisClientContract`).
...
- `Redis/RedisJobStateStore` — job state machine (claim, complete, fail, dead-letter, retry, fencing, heartbeat, zombie
  detection)
