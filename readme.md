...
- **Async** — `ASKRedisClient` sends commands through the ASK pipeline (Operation DTOs → ASKClient → Transport → TCP
  socket with RESP protocol)
- **Sync** — `PhpRedisAdapter` wraps the phpredis extension directly via `RedisClientContract`

Infrastructure classes (cache, lockers, queue, job state) use the sync path and implement contracts from
`bagart/async-kernel` and `bagart/ask-client`.
